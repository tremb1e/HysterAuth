...
class DataDecompressor:
    ...
    def _decompress_lz4(data: bytes) -> bytes:
        # Try standard LZ4 frame first.
        try:
            return lz4.frame.decompress(data)
        except Exception:
            # Compatibility: some clients use "length prefix + raw block" format.
            if len(data) > 4:
            ...
            # If there is no length prefix, re-raise so the caller can try other strategies.
            raise
