"""Legacy experiment: VQ autoencoder trainer (DataParallel; VQ is used in the main pipeline; discriminator code is legacy)."""
...
class TrainVQAutoencoder:
    def __init__(self, args, user_id, total_far, total_frr, total_err, total_f1, total_auc):
        ...
        '''
        self.vq.load_checkpoint(args.checkpoint_path)
    ...
    def configure_optimizers(self, args):
        ...
        opt_vq = torch.optim.Adam(
            list(self.vq.module.encoder.parameters()) +
            list(self.vq.module.decoder.parameters()) +
            list(self.vq.module.codebook.parameters()) +
            list(self.vq.module.quant_conv.parameters()) +
            list(self.vq.module.post_quant_conv.parameters()),
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, (imgs,_) in zip(pbar, train_dataset):
                    ...
                    disc_factor = self.vq.module.adopt_weight(args.disc_factor, epoch*steps_per_epoch+i, threshold=args.disc_start)
                    ...
                    λ = self.vq.module.calculate_lambda(perceptual_rec_loss, g_loss)
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(90), desc="total user"):
        user_id = i
        train_vq = TrainVQAutoencoder(args, user_id, total_far, total_frr, total_err, total_f1, total_auc)
