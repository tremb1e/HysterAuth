...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        attn_resolutions = []  # NonLocal at the bottleneck is usually sufficient.
        num_res_blocks = 2
        resolution = 12  # Roughly track feature-map height.
        layers = [nn.Conv2d(args.image_channels, channels[0], 3, 1, 1)]
        downsample_plan = [
            {"stride": (2, 2), "pad": (0, 1, 0, 1)},  # 12x50 -> 6x25
            {"stride": (1, 2), "pad": (0, 1, 1, 1)},  # 6x25 -> 6x12 (downsample time axis only)
            {"stride": (1, 2), "pad": (0, 1, 1, 1)},  # 6x12 -> 6x6 (yields 36 tokens)
        ]
        for i in range(len(channels)-1):
            ...
            if i < len(downsample_plan):
                ...
                # Roughly track resolution, mainly to decide whether to add a NonLocalBlock.
